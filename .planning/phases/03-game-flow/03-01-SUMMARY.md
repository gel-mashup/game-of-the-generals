...
tags: [game-engine, win-conditions, socket, rematch, tdd]

# Dependency graph
requires:
  - phase: 02-game-core
    provides: game engine pure functions, socket handlers, Room type
provides:
  - checkFlagCapture, checkFlagBaseline, checkNoValidMoves, checkWinCondition pure functions
  - game:over socket event emission after each move
  - rematch socket handler with confirm flow and 30s timeout
  - reset-scores socket event (host-only)
  - scores:update emitted on game over and rematch/reset
affects: [03-game-flow, 04-ai-opponent]

# Tech tracking
tech-stack:
  added: [checkWinCondition, WinResult type, rematchHandler]
  patterns: [pure function win detection, socket event-driven game flow, in-memory Room state extension]

key-files:
  created: [server/src/socket/handlers/rematchHandler.ts]
  modified:
...
requirements-completed: [WIN-01, WIN-02, WIN-03, WIN-04, SES-02, SES-03]
